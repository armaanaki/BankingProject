...
public class ConsoleManager {
	...
	private static BankAccountDAO sqlAccess = new BankAccountDAO();
	
	//first stage to select a new account or existing one
	public static void introSelection() {
		referenceAccount.setTotalAccounts(sqlAccess.getTotalAccounts());
		while (true) {
	...
	private static BankAccount validateBankAccount() {
		...
		while (true) {
			...
			if (userInput.hasNextInt()) {
				...
				String accountHolder = sqlAccess.getAccountHolderName(accountNumber, accountPassword);
				double totalInterest = sqlAccess.getTotalInterest(accountNumber, accountPassword);
				String accountType = sqlAccess.getAccountType(accountNumber, accountPassword);
				boolean overdraft = sqlAccess.getOverdraft(accountNumber, accountPassword);
				double accountBalance = sqlAccess.getAccountBalance(accountNumber, accountPassword);
				if (!accountHolder.isEmpty()) {
					if (accountType.equals("checkings")) {
						return new CheckingsAccount(accountHolder, accountNumber, accountBalance, accountPassword, overdraft);
					} else {
						return new SavingsAccount(accountHolder, accountNumber, accountBalance, accountPassword, totalInterest);
					}
				} else {
					System.out.println(invalidInput);
				}
			...
			switch (input) {
				...
				case "2": 
					if (referenceAccount.getTotalAccounts()>1 && userAccount.getAccountBalance()>0) {
						...
						updateDatabase(transferAccount);
					} else {
	...
	private static void createSavingsAccount() {
		referenceAccount.createdNewAccount();
		BankAccount user = new SavingsAccount(inputAccountHolder(), referenceAccount.getTotalAccounts(), inputAccountTotal(), inputAccountPassword(), 0);
		sqlAccess.addAccount(user.getAccountHolderName(), user.getAccountNumber(), user.getAccountBalance(), user.getAccountType(),
				user.getAccountPassword(), false, ((SavingsAccount) user).getTotalInterest());
	...
	private static void createCheckingsAccount() {
		referenceAccount.createdNewAccount();
		BankAccount user = new CheckingsAccount(inputAccountHolder(), referenceAccount.getTotalAccounts(), inputAccountTotal(), inputAccountPassword(), inputOverdraft());
		sqlAccess.addAccount(user.getAccountHolderName(), user.getAccountNumber(), user.getAccountBalance(), user.getAccountType(), 
				user.getAccountPassword(), ((CheckingsAccount) user).getOverdraft(), 0);
		updateDatabase(user);
		System.out.println("Congratulations " + user.getAccountHolderName() + "\nYour account number is: " + user.getAccountNumber()
			+ "\nYour account password is: " + user.getAccountPassword());
	}
	...
	private static void successfulTransaction(BankAccount userAccount){
		if (userAccount.getAccountType().equals("savings")) {
			System.out.println(String.format("Your transaction was a success! Your new balance is: $%,.2f \nYour total interest is: $%,.2f", 
					userAccount.getAccountBalance(), ((SavingsAccount) userAccount).getTotalInterest()));
		} else {
			System.out.println(String.format("Your transaction was a success! Your new balance is: $%,.2f", userAccount.getAccountBalance()));
		}
	}
	
	public static void updateDatabase(BankAccount user) {
		if (user.getAccountType().equals("checkings")) {
			sqlAccess.updateAccount(user.getAccountHolderName(), user.getAccountNumber(), user.getAccountBalance(), user.getAccountType(), 
					user.getAccountPassword(), ((CheckingsAccount) user).getOverdraft(), 0, user.getTotalAccounts());
		} else {
			sqlAccess.updateAccount(user.getAccountHolderName(), user.getAccountNumber(), user.getAccountBalance(), user.getAccountType(), 
					user.getAccountPassword(), false, ((SavingsAccount) user).getTotalInterest(), user.getTotalAccounts());
		}
	}
}